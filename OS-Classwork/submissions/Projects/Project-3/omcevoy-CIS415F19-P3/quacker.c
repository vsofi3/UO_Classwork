...
int ts_getEntry(int idNo, int lastEntry, topicEntry* empty) {
    int i = 0;
    int rval = 0;                   
    for (i; i < maxTopics; i++) { //check if queue exists
        if (idNo == topics[i].id) {
            pthread_mutex_lock(&topics[i].mutex);
            rval = getEntry(topics[i].topic, lastEntry, empty);
            pthread_mutex_unlock(&topics[i].mutex);
        }
    }    
    return rval; 
...
int ts_dequeue(char* topic){
    ...
    for (i; i < maxTopics; i++) { //check if queue exists
        if (strcmp(topic, topics[i].topic) == 0) {
            ...
            topicEntry ty;
            topicEntry* empty = &ty;
            rval = dequeue(topics[i].topic, empty);
            pthread_mutex_unlock(&topics[i].mutex);
        }
    }    
    return rval; 
        ...
        if (strcmp(inputs[0], "put") == 0) {
            ...
            for (i = 0; i < maxTopics; i++) {
                if (topicId == topics[i].id) {
                    ...
                    if(enqueue(topics[i].topic, new))
                        printf("Proxy Thread %d - Type: Publisher - Executed Command: Enqueue\n", pthread_self());
                }
            }                                        
        }
...
void* subscriber(void* args) {
    ...
    pthread_mutex_unlock(&mtx);  
    tArg* sub = (tArg*) args;
    char* newFile = stringStrip(sub->file);
    strcpy(sub->file, newFile);    
    FILE* subFile = fopen(sub->file, "r");
    if (subFile == NULL) {exit(EXIT_FAILURE);}
    char* line = NULL;
    char* token;
    char* dummy;
    size_t len = 720;
    ssize_t nread; 
    while ((nread = getline(&line, &len, subFile) != -1 )) {
        token = strtok_r(line, " ", &dummy);
        int i = 0;
        char* inputs[len];
        while (token) {
        ...
        if (strcmp(inputs[0], "get") == 0) {
            int topId = atoi(inputs[1]);
            topicEntry ty;
            topicEntry* empty = &ty; 
            if(ts_getEntry(topId, lastEntry, empty));
                printf("Proxy Thread %d - Type: Subscriber - Executed Command: GetEntry\n", pthread_self());
            lastEntry = empty->entryNum;            
        }
        else if (strcmp(inputs[0], "sleep") == 0) {
            int sleepTime = atoi(inputs[1]);
            usleep(sleepTime);
...
void* clean_up() {
    ...
    while(masterKey) {
        ...
        int check = 0;
        for (i; i < maxTopics; i++) {
            if (ts_dequeue(topics[i].topic)) {
                printf("Proxy Thread %d - Type: Clean Up - Executed Command Dequeue\n", pthread_self());
                check++;
                sched_yield();
            }
        }
        if (check + numThreads == 0)
            masterKey = 0;                
    }   
    return;
}
