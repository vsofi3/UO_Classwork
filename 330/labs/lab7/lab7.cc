...
//    friend std::ostream & operator<<(std::ostream &s, IntQueue *Q);
  //  friend Mem::IntQueue & operator<<(IntQueue &Q, int x);
    //friend Mem::IntQueue & operator>>(IntQueue &Q);
  };
...
std::ostream &
Mem::IntQueue::operator<<(std::ostream &s; IntQueue *Q){
	if (Q == NULL) {
...
IntQueue &
Mem::IntQueue::operator<<(IntQueue *Q, int x){
	Q << Enqueue(x); 
}
//
// Test program 
